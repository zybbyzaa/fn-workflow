  ...
  if (ext === '.json') {
    ...
    if (_.isObject(json)) {
      ...
      Object.keys(json).forEach(function(key) {
        ...
        var cleanReplacement = path
          .basename(json[key])
          .replace(new RegExp(opts.revSuffix), '');
        if (
          !~[
            path.basename(key),
...
function closeDirBySep(dirname) {
  return (
    dirname +
    (!dirname || new RegExp(escPathPattern('/') + '$').test(dirname) ? '' : '/')
  );
}
...
function revCollector(opts) {
  ...
  return through.obj(
    ...
    function(cb) {
      ...
      for (var key in manifest) {
        ...
        if (opts.replaceReved) {
          ...
          if (patternExt in opts.extMap) {
            patternExt =
              '(' +
              escPathPattern(patternExt) +
              '|' +
              escPathPattern(opts.extMap[patternExt]) +
              ')';
          } else {
      ...
      // 修改匹配顺序，优先匹配更短的路径
      // Replace shorter patterns first
